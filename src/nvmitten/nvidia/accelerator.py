...
from ..systems.accelerator import Accelerator
from ..systems.info_source import InfoSource, INFO_SOURCE_REGISTRY
from ..utils import run_command


# TODO: Integrate nvmitten.nvidia.smi and nvmitten.nvidia.dla
...
def get_gpu_info(soc_model_filepath: str = SOC_MODEL_FILEPATH,
    ...
    Args:
        soc_model_filepath (str): The path to the 'model' file containing the name of the SoC's model. This method and
                                  ...
                                  internally in INFO_SOURCE_REGISTRY.get(("accelerators", "nvgpu")). (Default: SOC_MODEL_FILEPATH)
        force_no_gpu_cmd (bool): Set to True to skip dGPU detection. Only used for testing. This method and arguments
                                 are not exposed outside of the systems module API. (Default: False)
...
class GPU(Accelerator):
    ...
    @classmethod
    def detect(cls):
        """Grabs the next GPU info from INFO_SOURCE_REGISTRY.get(("accelerators", "nvgpu")). The caller must maintain
        INFO_SOURCE_REGISTRY.get(("accelerators", "nvgpu")) and make sure it is reset before calling it.

        Returns:
            GPU: A GPU object with fields from the top item from the InfoSource buffer
        """
        gpu_fields = next(INFO_SOURCE_REGISTRY.get(("accelerators", "nvgpu")))
...
def get_mig_info(force_no_gpu_cmd=False, skip_sm_check=False, force_sm_value: Optional[int] = None) -> List[Dict[str, str]]:
    ...
    INFO_SOURCE_REGISTRY.get("nvidia_smi").reset()
    for gpu, migs in INFO_SOURCE_REGISTRY.get("nvidia_smi"):
...
class MIG(GPU):
    ...
    @classmethod
    def detect(cls):
        """Grabs the next MIG instance info from INFO_SOURCE_REGISTRY.get(("accelerators", "nvmig")). The caller must
        maintain INFO_SOURCE_REGISTRY.get(("accelerators", "nvmig")) and make sure it is reset before calling it.

        Returns:
            MIG: A MIG object with fields from the top item from the InfoSource buffer
        """
        mig_fields = next(INFO_SOURCE_REGISTRY.get(("accelerators", "nvmig")))
...
INFO_SOURCE_REGISTRY.register("nvidia_smi", InfoSource(get_accelerator_info))
INFO_SOURCE_REGISTRY.register(("accelerators", "nvgpu"), InfoSource(get_gpu_info, klass=GPU))
INFO_SOURCE_REGISTRY.register(("accelerators", "nvmig"), InfoSource(get_mig_info, klass=MIG))
